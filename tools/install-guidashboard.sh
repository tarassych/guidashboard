...
deploy_frontend() {
    ...
    # Check for Google Maps API key from environment variable or existing .env
    local maps_api_key=""
    local env_file="$REPO_DIR/.env"
    
    # Priority: 1) Environment variable, 2) Existing .env file, 3) Prompt user
    if [ -n "${GOOGLE_MAPS_API_KEY:-}" ]; then
        maps_api_key="$GOOGLE_MAPS_API_KEY"
        print_success "Google Maps API key found (from environment)"
    elif [ -f "$env_file" ] && grep -q "VITE_GOOGLE_MAPS_API_KEY=" "$env_file" 2>/dev/null; then
        maps_api_key=$(grep "VITE_GOOGLE_MAPS_API_KEY=" "$env_file" | cut -d'=' -f2)
        if [ -n "$maps_api_key" ]; then
            print_installed "Google Maps API key (from existing .env)"
        fi
    fi
    
    # If no key found, prompt user (unless in non-interactive mode)
    if [ -z "$maps_api_key" ]; then
        echo ""
        print_warning "Google Maps API key not configured"
        print_detail "Map panel will show 'For development purposes only' watermark"
        echo ""
        
        if [ "$SKIP_CONFIRM" != true ] && [ -t 0 ]; then
            echo -e "  ${WHITE}Enter Google Maps API key (or press Enter to skip):${NC}"
            read -r maps_api_key
            
            if [ -n "$maps_api_key" ]; then
                print_success "API key entered"
            else
                print_info "Skipped - you can add it later to $env_file"
            fi
        else
            print_info "Non-interactive mode - skipping API key prompt"
            print_detail "Set GOOGLE_MAPS_API_KEY env variable before running installer"
        fi
    fi
    
    # Create/update .env file
    start_spinner "Creating frontend .env configuration"
    cat > "$env_file" << EOF
# GUI Dashboard Frontend Configuration
# Generated by install-guidashboard.sh

# API URL (empty for production - uses same host)
VITE_API_URL=

# Default theme: digital-green or aggressive-red
VITE_DEFAULT_THEME=digital-green

# Google Maps API Key for satellite map view
VITE_GOOGLE_MAPS_API_KEY=${maps_api_key}
EOF
    chown orangepi:orangepi "$env_file"
    stop_spinner
...
verify_installation() {
    ...
    if [ "$all_ok" = false ]; then
        ...
        echo -e "    pm2 logs guidashboard-api    - View backend logs"
        echo -e "    sudo systemctl status nginx  - Check nginx"
        echo -e "    sudo systemctl status mediamtx - Check mediamtx"
        echo ""
        return 1
    fi
